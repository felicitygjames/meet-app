...
const getToken = async (code) => {
  ...
  const { access_token } = await fetch(
    "https://m8e37474il.execute-api.us-west-1.amazonaws.com/dev/api/token/" +
      encodeCode
  )
    .then((res) => {
      return res.json();
...
export const getEvents = async () => {
  ...
  if (token) {
    removeQuery();
    const url =
      "https://m8e37474il.execute-api.us-west-1.amazonaws.com/dev/api/get-events/" +
      token;
  ...
  if (!accessToken || tokenCheck.error) {
    ...
    if (!code) {
      const results = await axios.get(
        "https://m8e37474il.execute-api.us-west-1.amazonaws.com/dev/api/get-auth-url"
      );
